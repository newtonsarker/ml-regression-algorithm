...
This module provides functions to perform gradient descent for linear regression models.

Functions:
    derivative_of_cost_of_single_feature_model(vector_of_feature_x, vector_of_output_y, weight, bias):
        Computes the gradient for linear regression with a single feature.
        Returns the derivatives of the cost with respect to the parameters weight and bias.

    derivative_of_cost_of_multiple_feature_model(vector_of_features_x, vector_of_output_y, vector_of_weight, bias):
        Computes the gradient for linear regression with multiple features.
        Returns the derivatives of the cost with respect to the parameters weights and bias.

    gradient_descent_of_single_feature_model(vector_of_feature_x, vector_of_output_y, initial_weight, initial_bias, learning_rate_alpha, no_of_iterations):
        Performs gradient descent to fit the model parameters for a single-feature model.
        Updates the parameters by taking no_of_iterations gradient steps with learning rate alpha.

    gradient_descent_of_multiple_feature_model(vector_of_features_x, vector_of_output_y, initial_weight, initial_bias, learning_rate_alpha, no_of_iterations):
        Performs gradient descent to fit the model parameters for a multiple-feature model.
        Updates the parameters by taking no_of_iterations gradient steps with learning rate alpha.

Usage:
    The functions in this module are used in the linear regression process to compute the gradients
    and update the model parameters during the training process.

    Example usage in linear regression:
        from gradient_descent import derivative_of_cost_of_single_feature_model, derivative_of_cost_of_multiple_feature_model, gradient_descent_of_single_feature_model, gradient_descent_of_multiple_feature_model

        # Compute the gradient for single-feature model
        dj_dw, dj_db = derivative_of_cost_of_single_feature_model(vector_of_feature_x, vector_of_output_y, weight, bias)

        # Compute the gradient for multiple-feature model
        dj_dw, dj_db = derivative_of_cost_of_multiple_feature_model(vector_of_features_x, vector_of_output_y, vector_of_weight, bias)

        # Perform gradient descent for single-feature model
        w, b, cost_history = gradient_descent_of_single_feature_model(vector_of_feature_x, vector_of_output_y, initial_weight, initial_bias, learning_rate_alpha, no_of_iterations)

        # Perform gradient descent for multiple-feature model
        w, b, cost_history = gradient_descent_of_multiple_feature_model(vector_of_features_x, vector_of_output_y, initial_weight, initial_bias, learning_rate_alpha, no_of_iterations)
"""
import copy
import numpy as np
